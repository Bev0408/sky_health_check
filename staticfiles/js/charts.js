...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('categoryHealthChart')) {
...
function initTrendsChart() {
    ...
    const chartDataElement = document.getElementById('trends-chart-data');
    if (!chartDataElement) return;
    
    const labels = JSON.parse(chartDataElement.getAttribute('data-labels'));
    const redData = JSON.parse(chartDataElement.getAttribute('data-red'));
    const yellowData = JSON.parse(chartDataElement.getAttribute('data-yellow'));
    const greenData = JSON.parse(chartDataElement.getAttribute('data-green'));
...
function initCategoryHealthChart() {
    const ctx = document.getElementById('categoryHealthChart').getContext('2d');
    
    // Get data from the data attributes
    const chartDataElement = document.getElementById('category-health-data');
    if (!chartDataElement) return;
    
    const categories = JSON.parse(chartDataElement.getAttribute('data-categories'));
    const redData = JSON.parse(chartDataElement.getAttribute('data-red'));
    const yellowData = JSON.parse(chartDataElement.getAttribute('data-yellow'));
    const greenData = JSON.parse(chartDataElement.getAttribute('data-green'));
